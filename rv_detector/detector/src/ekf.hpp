...
class EKF : public Filter {
 ...
 public:
  explicit EKF(const Matx51d& Xe = Matx51d::zeros());
  ~EKF();
  void SetDeltaT(double delta_t);
  void Init(const std::vector<double>& vec);

  const cv::Mat& Predict(const EMatx51d& measurements);
  const cv::Mat& Update(const EMatx31d& measurements);
  void Estimate();
  bool Config(EMatx51d& measurements, unsigned int frame_count,
              double kT);
};
